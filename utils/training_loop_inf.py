...
def training_loop_inf(
    ...
    optimizer_loss = torch.optim.AdamW(criterion.parameters(), lr=0.0001, eps=1e-06)
    scaler = GradScaler()
    ...
    for i, (images, labels) in enumerate(train_pbar):
        ...
        with autocast(dtype=torch.float16):
            ...
            loss,_,kg_loss,_, only_sea = criterion(outputs, labels)
            if only_sea:
                loss = kg_loss
                print('only sea')
        ...
        train_pbar.set_postfix({
            "loss": f"{train_loss / (i%steps_per_epoch + 1):.4f}",
        ...
        if (i+1)%steps_per_epoch == 0:
            ...
            with torch.no_grad():
                ...
                for j, (images, labels) in enumerate(val_loader):
                    ...
                    loss, coord_loss, kg_loss, time_loss, only_sea = criterion(outputs, labels)
                    if only_sea:
                        loss = kg_loss
            ...
            train_pbar.set_postfix({
                ...
                **{name: f"{value / (steps_per_epoch):.4f}" for name, value in train_metrics_values.items()},
                "val_l": f"{val_loss / (j + 1):.4f}","c_l": f"{running_coords / (j + 1):.4f}","t_l": f"{running_time / (j + 1):.4f}","kg_l": f"{running_kg / (j + 1):.4f}","t_l": f"{running_time / (j + 1):.4f}",
            ...
            if best_loss is None:
                ...
                np.save(os.path.join(out_folder, f"{name}_best_centers.npy"), criterion.directions.detach().cpu().numpy())
                np.save(os.path.join(out_folder, f"{name}_best_densities.npy"), criterion.densities.detach().cpu().numpy())
            ...
            np.save(os.path.join(out_folder, f"{name}_last_centers.npy"), criterion.directions.detach().cpu().numpy())
            np.save(os.path.join(out_folder, f"{name}_last_densities.npy"), criterion.densities.detach().cpu().numpy())
            ...
            if visualise_validation:
                visualise(vis_images, np.squeeze(vis_labels), np.squeeze(vis_preds), images=num_visualisations,
                            channel_first=True, vmin=0, vmax=1, save_path=os.path.join(save_dir, f"val_pred_{epoch}.png"), centers=criterion.directions.detach().cpu().numpy())
